...
class TutorVCUpdate(commands.Cog):
    def __init__(self, bot):
        ...
        self.channel_id = {MAIN_ID.g_main: MAIN_ID.ch_controlPanel, STAFF_ID.g_staff: STAFF_ID.ch_console}
        self.staticChannels = [MAIN_ID.ch_startPrivateVC, STAFF_ID.ch_startPrivateVC]
        self.presetChannels = [MAIN_ID.ch_controlPanel, MAIN_ID.ch_startPrivateVC, STAFF_ID.ch_console, STAFF_ID.ch_startPrivateVC]
        ...
        self.CcategoryIDs = {MAIN_ID.g_main: MAIN_ID.cat_privateVC, STAFF_ID.g_staff: STAFF_ID.cat_privateVC}
    ...
    async def on_voice_state_update(self, member, before, after):
        ...
        if before.channel is not None and (after.channel is None or after.channel.category_id not in self.categoryIDs or
                                           after.channel.id in self.staticChannels) and not member.bot:
            ...
            ignoreQuery = database.IgnoreThis.select().where((database.IgnoreThis.authorID == member.id) &
                                                             (database.IgnoreThis.channelID == before.channel.id) &
                                                             (database.IgnoreThis.GuildID == before.channel.guild.id))
            
            if ignoreQuery.exists():
                iq: database.IgnoreThis = database.IgnoreThis.select().where((database.IgnoreThis.authorID == member.id) &
                                                                             (database.IgnoreThis.channelID == before.channel.id) &
                                                                             (database.IgnoreThis.GuildID == before.channel.guild.id)).get()
                iq.delete_instance()
            ...
            if query.exists() and before.channel.category.id in self.categoryIDs:
                query = database.VCChannelInfo.select().where((database.VCChannelInfo.authorID == member.id) &
                                                              (database.VCChannelInfo.ChannelID == before.channel.id) &
                                                              (database.VCChannelInfo.GuildID == before.channel.guild.id)).get()
                try:
                ...
                if query.ChannelID == str(before.channel.id):
                    embed = discord.Embed(title = f"{Emoji.time} WARNING: Voice Channel is about to be deleted!",
                    ...
                    if member in before.channel.members:
                        ...
                        if query.exists():
                            ...
                            try:
                                ...
                                embed = discord.Embed(title = f"{Emoji.archive} {member.display_name} Total Voice Minutes",
                                ...
                                if tutorSession.exists():
                                    ...
                                    hourlog = discord.Embed(title = "Hour Log",
                                    ...
                                    feedback = discord.Embed(title = "Feedback!",
                                    ...
                                    loggedhours = discord.Embed(title = "Logged Hours",
        ...
        if after.channel is not None and after.channel.id in self.presetChannels and not member.bot:
            ...
            if len(category.voice_channels) >= 25:
                embed = discord.Embed(title = f"{Emoji.deny} Max Channel Allowance",
            ...
            if query.exists():
                moveToChannel = database.VCChannelInfo.select().where((database.VCChannelInfo.authorID == member.id) &
                                                                      (database.VCChannelInfo.GuildID == after.channel.guild.id)).get()
                embed = discord.Embed(title = f"{Emoji.deny} Maximum Channel Ownership Allowance",
                ...
                if not any(role in member.roles for role in roleList):
                    ...
                    embed = discord.Embed(title = f"{Emoji.confirm} Voice Channel Creation",
                ...
                if TutorRole in member.roles:
                    embed.add_field(name = "Convert to Tutor Session?",
