...
class TutorVCCMD(commands.Cog):
    ...
    async def startVC(self, ctx):
        ...
        embed.add_field(name = "Creating a Private Channel", value = f"The **only** way to create a voice channel is through joining <#{self.StartVCIDs[ctx.guild.id]}>. There is **no command**, so please don't spam `+start` or whatever comes up in your head. ")
