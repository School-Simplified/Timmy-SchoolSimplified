...
class TutorVCCMD(commands.Cog):
    def __init__(self, bot: commands.Bot):
        ...
        # Level Roles
        self.leveledRoles = ["〚Level 120〛Grandmaster", "〚Level 110〛Master", "〚Level 100〛Prodigy", "〚Level 90〛 Legend",
                             "〚Level 80〛Connoisseur", "〚Level 70 〛Professor", "〚Level 60〛Mentor", "〚Level 50〛Scholar",
                             "〚Level 40〛Expert", "〚Level 35〛Experienced", "〚Level 30〛Apprentice", "〚Level 25〛Amateur",
                             "〚Level 20〛Student", "〚Level 15〛Learner", "〚Level 10〛Beginner", "〚Level 5〛Novice",
                             "〚Level 1〛New"]
        self.renameRoles = ["〚Level 120〛Grandmaster", "〚Level 110〛Master", "〚Level 100〛Prodigy", "〚Level 90〛 Legend",
                            "〚Level 80〛Connoisseur", "〚Level 70 〛Professor", "〚Level 60〛Mentor",
                            "〚Level 50〛Scholar", "〚Level 40〛Expert", "Simplified Booster", "Legend", "Moderator",
        ...
        self.LobbyStartIDs = {MAIN_ID.g_main: MAIN_ID.ch_controlPanel, STAFF_ID.g_staff: STAFF_ID.ch_console}
        self.StartVCIDs = {MAIN_ID.g_main: MAIN_ID.ch_startPrivateVC, STAFF_ID.g_staff: STAFF_ID.g_staff}
    ...
    async def rename(self, ctx, *, name = None):
        ...
        roleList = [SB, AT, legend, MT, MAT, TT, VP, CO, L120, L110, L100, L90, L80, L70, L60, L50, L40]

        member = ctx.guild.get_member(ctx.author.id)

        if ctx.author.id != 415629932798935040:
            if not any(role in ctx.author.roles for role in roleList) and ctx.guild.id == 763119924385939498:
                embed = discord.Embed(title = f"{Emoji.deny} Insufficient Rank",
            ...
            if member.voice.channel.category_id in self.categoryID:
                ...
                if query.exists():
                    ...
                    if member.voice.channel.id == int(q.ChannelID):
                        if name is not None:
                            embed = discord.Embed(title = f"{Emoji.cycle} ReNamed Channel",
                        ...
                        embed = discord.Embed(title = f"{Emoji.deny} Ownership Check Failed",
                ...
                embed = discord.Embed(title = f"{Emoji.invalidchannel} Unknown Channel",
    ...
    async def end(self, ctx):
        ...
        if voice_state is None:
            ...
            if query.exists():
                ...
                embed = discord.Embed(title = f"{Emoji.archive} Ended Session",
                                      description = "I have successfully ended the session!",
                                      color = discord.Colour.blue())
                embed.add_field(name = "Time Spent", value = f"{member.mention} you have spent a total of {Emoji.calender} "
                ...
                if tutorSession.exists():
                    ...
                    hourlog = discord.Embed(title = "Hour Log", description = f"{tutor.mention}'s Tutor Log",
                                            color = discord.Colour.blue())
                    hourlog.add_field(name = "Information",
        ...
        if voice_state.channel.id in self.presetChannels:
            embed = discord.Embed(title = f"{Emoji.invalidchannel} UnAuthorized Channel Deletion",
        ...
        if member.voice.channel.category_id in self.categoryID:
            ...
            if query.exists():
                q: database.VCChannelInfo = database.VCChannelInfo.select().where((database.VCChannelInfo.authorID == ctx.author.id) &
                                                                                  (database.VCChannelInfo.ChannelID == voice_state.channel.id) &
                                                                                  (database.VCChannelInfo.GuildID == ctx.guild.id)).get()
                ...
                if tutorSession.exists():
                    ...
                    embed = discord.Embed(title = "Logged Hours",
                ...
                try:
                    q = database.VCChannelInfo.select().where((database.VCChannelInfo.ChannelID == voice_state.channel.id) &
                                                              (database.VCChannelInfo.GuildID == ctx.guild.id)).get()
                except:
                    embed = discord.Embed(title = f"{Emoji.invalidchannel} Ownership Check Failed",
    ...
    async def forceend(self, ctx, channel):
        ...
        if channel.category_id in self.categoryID:
            ...
            if query.exists():
                q: database.VCChannelInfo = database.VCChannelInfo.select().where((database.VCChannelInfo.ChannelID == channel.id) &
                                                                                  (database.VCChannelInfo.GuildID == ctx.guild.id)).get()
                VCDatetime = pytz.timezone("America/New_York").localize(q.datetimeObj)
                ...
                await channel.delete()
                q.delete_instance()
                embed = discord.Embed(title = f"{Emoji.archive} Force Ended Session",
                                      description = "Session has been forcefully removed.",
                                      color = discord.Colour.blue())
                embed.add_field(name = "Time Spent", value = f"<@{q.authorID}> you have spent a total of {Emoji.calender} "
                                                             f"`{day} minutes` in voice channel, **{q.name}**.")
                embed.set_footer(text = "WARNING: Time displayed may not be accurate.")
                await ctx.send(embed = embed)
                ...
                await channel.delete()
                embed = discord.Embed(title = f"{Emoji.warn} Partial Completion",
            ...
            embed = discord.Embed(title = f"{Emoji.warn} Unknown Channel",
                                  description = "You are not the owner of this voice channel nor is this a valid channel. "
                                                "Please execute the command under a valid voice channel!", color = discord.Colour.red())
            await ctx.send(embed = embed)
    ...
    async def lock(self, ctx):
        ...
        if voice_state is None:
            embed = discord.Embed(title = f"{Emoji.warn} Unknown Voice Channel",
            ...
            if voice_state.channel.id in self.presetChannels:
                embed = discord.Embed(title = f"{Emoji.deny} UnAuthorized Channel Modification",
            ...
            if member.voice.channel.category_id in self.categoryID:
                ...
                if query.exists():
                    LOCK : database.VCChannelInfo = database.VCChannelInfo.select().where((database.VCChannelInfo.authorID == ctx.author.id) &
                                                                                          (database.VCChannelInfo.ChannelID == voice_state.channel.id) &
                                                                                          (database.VCChannelInfo.GuildID == ctx.guild.id)).get()
                    LOCK.lockStatus = "1"
                    LOCK.save()
                    ...
                    embed = discord.Embed(title = f"{Emoji.lock} Locked Voice Channel",
            ...
            if tutorSession.exists():
                ...
                if member.voice.channel.category_id in self.categoryID:
                    ...
                    if query.exists():
                        ...
                        hourlog = discord.Embed(title = "Tutor Session Convert Complete",
        ...
        if voice_state is None:
            ...
            if member.voice.channel.category_id in self.categoryID:
                ...
                if query.exists():
                    ...
                    embed = discord.Embed(title = f"{Emoji.unlock} Unlocked Voice Channel",
            ...
            if member.voice.channel.category_id in self.categoryID:
                ...
                if query.exists():
                    ...
                    if query.lockStatus == "0":
                        embed = discord.Embed(title = f"{Emoji.deny} Invalid Setup",
                        ...
                        if typeAction == "+" or typeAction.lower() == "add":
                            ...
                            embed = discord.Embed(title = f"{Emoji.minusgear} Permit Setup",
                            ...
                            embed = discord.Embed(title = f"{Emoji.cycle} Permit List",
                                                  description = f"**Users Authorized:**"
                                                                f"\n\n{formatVer}",
                                                  color = discord.Color.gold())
                            await ctx.send(embed = embed)

                        else:
                            embed = discord.Embed(title = f"{Emoji.warn} Invalid Operation",
    ...
    async def voicelimit(self, ctx, new_voice_limit):
        ...
        if voice_state is None:
            embed = discord.Embed(title = f"{Emoji.invalidchannel} Unknown Voice Channel",
                                  description = "You have to be in a voice channel you own in order to use this!",
                                  color = discord.Colour.dark_red())
            return await ctx.send(embed = embed)
        ...
        if voice_state is None:
            ...
            if member.voice.channel.category_id in self.categoryID:
                ...
                if query.exists():
                    ...
                    embed = discord.Embed(title = f"{Emoji.minusgear} Disconnected User",
    ...
    async def startVC(self, ctx):
        embed = discord.Embed(title = "Private Channels",
                              description = "First off, there is **no** command to start a private channel.",
                              color = discord.Color.green())
        embed.add_field(name = "Creating a Private Channel",
        ...
        embed.add_field(name = "Permissions",
                        ...
                        inline = False)
        embed.add_field(name = "Rank Requirements",
