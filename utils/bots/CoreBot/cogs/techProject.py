...
async def createChannel(self, ctx: commands.Context, type: str, member: discord.Member, discordEmbed: discord.Embed):
    if type == "Developer Team":
        ...
        Title = "developer"
        embed = discord.Embed(title = "Developer Ticket",
                              description = f"Welcome {member.mention}! A developer will be with you shortly.",
                              color = discord.Color.green())
        category = discord.utils.get(ctx.guild.categories, id= TECH_ID.cat_developerComms)
...
class TechProjectCMD(commands.Cog):
    ...
    async def techembedc(self, ctx):
        ...
        embed.add_field(name = "Developer Commissions",
        ...
        embed.add_field(name = "Discord Commissions",
    ...
    async def request(self, ctx: commands.Context):
        ...
        embed = discord.Embed(title = "Q1: What is a descriptive title for your project?",
                              color = discord.Colour.gold())
        await channel.send(embed = embed)
        answer1 = await self.bot.wait_for('message', check=check)

        embed = discord.Embed(title = "Q2: Which of these categories does your project suggestion fit under?",
                              color = discord.Colour.gold())
        
        options = [
            discord.SelectOption(label='Developer Team',
                                 description='If you need a Discord Bot or a modification to one, click here!',
                                 emoji='🤖'),
            discord.SelectOption(label='Website Team',
        ...
        if answer2 == "Website Team":
            embed = discord.Embed(title = "Website Team Commissions",
        ...
        embed = discord.Embed(title = "Q5: Have you received approval from a manager for this project (or are you a manager yourself)?",
                              color = discord.Colour.gold())
        await channel.send(embed = embed)
        ...
        embed = discord.Embed(title = "Confirm Responses...", description = "Are you ready to submit these responses?",
                              color = discord.Colour.gold())
        ...
        if buttonView.value is None:
            ...
            NPR = discord.Embed(title = "New Project Request", description = f"Project Requested by {ctx.author.mention}",
            ...
            NPR.add_field(name = "Q5: Have you received approval from a manager for this project (or are you a manager yourself)?",
    ...
    async def projectR(self, ctx, user: discord.User, type, projectname, *, notes = None):
        embed = discord.Embed(title = "Project Announcement",
