...
class RedirectURL(commands.Cog):
    def __init__(self, bot):
        ...
        self.raOBJ = redirect_sdk.RedirectClient(os.getenv("RP_TK"), domain="https://rs.schoolsimplified.org")
    ...
    async def rl(self, ctx):
        ...
        newlist = []
        for obj in objlist:
            newlist.append(f"**ID:** {obj.id} | **URL:** `https://{obj.domain}/{obj.source}` -> `{obj.destination}`")
        newlist = "\n".join(newlist)
        embed = discord.Embed(title=f"Redirects for {self.raOBJ.domain}", color=discord.Color.blue())
