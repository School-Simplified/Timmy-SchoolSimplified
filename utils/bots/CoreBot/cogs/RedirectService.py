...
class RedirectURL(commands.Cog):
    ...
    async def ra(self, interaction: discord.Interaction, redirect_code: str, destination_url: str, sub_domain: str = None):
        if sub_domain is None:
            ...
            query = database.ApprovedSubDomains.select().where(database.ApprovedSubDomains.sub_domain == sub_domain)
            if query.exists():
                try:
                    val = self.raOBJ.add_redirect(redirect_code, destination_url, sub_domain)
                except redirect_sdk.UnprocessableEntity as e:
                    errors = "\n".join(e.errors[0])
                    embed = discord.Embed(title="Unprocessable Entity", color=discord.Color.brand_red())
                    embed.add_field(name="Unable to Add Redirect", value=errors)
