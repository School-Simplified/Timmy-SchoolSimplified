...
class GSuiteLogin(commands.Cog):
    ...
    async def GSuiteVerify(self, interaction: discord.Interaction):
        ...
        if (
            ...
            embed = discord.Embed(title = "GSuite Verification", description = "Click on the button below and sign in with your **personal** @schoolsimplified.org account.\nOnce you sign in you should get a code, copy and paste that here.", color = discord.Color.red())
            ...
            embed.set_footer(text = "NOTE: This token is not stored and is only used to determine your identity.")
            ...
            await DMChannel.send("Now paste your authentication code here:")
            ...
            try:
                ...
                if (
                    ...
                    try:
                        ...
                        respjson = session.get('https://www.googleapis.com/userinfo/v2/me').json()
                        firstname = respjson["given_name"]
                        if firstname == "SS" or firstname == "PS":
                            return await DMChannel.send("You are not allowed to authenticate with Department Accounts, please use your **personal** account to authenticate.")
                        else:
                            embed = discord.Embed(title = "Authentication Successful", description = f"Hello {firstname}, you have successfully authenticated with your GSuite account, {respjson['email']}.", color = discord.Color.green())
                            ...
                            await member.add_roles(role, reason = "Passed Verification: {}".format(respjson["email"]))
                            ...
                            embed.add_field(name = "Verification Details", value = "**Name:** {}\n**Email:** {}\n**Date:** {} EST".format(f"{respjson['given_name']} {respjson['family_name']}", respjson["email"], now))
                            embed.set_footer(text = "This action was performed by {}".format(member.display_name))
                            await logchannel.send(embed=embed)
            
            except asyncio.TimeoutError:
                await DMChannel.send("Looks like you didn't respond in time, please try again later!")
