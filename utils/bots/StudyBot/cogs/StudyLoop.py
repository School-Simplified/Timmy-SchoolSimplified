...
class StudyLoop(commands.Cog):
    def __init__(self, bot: commands.Bot):
        ...
        self.lastReset = parser.parse(
            "14.03.2022 00:00:00 EST", tzinfos={"EST": -4 * 3600}
        )  # 14.03.2022 as default value
    ...
    async def TTSWeekCheck(self):
        ...
        if (
            weekdayNow == 1
            and (now - self.lastReset >= datetime.timedelta(days=7))
            and timeNow >= self.midnight
        ):
            lastReset = now
