...
class InfoCMD(commands.Cog):
    ...
    async def info(self, ctx: commands.Context, user: commands.Greedy[discord.User] = []):
        for user in user:
            ...
            embed = discord.Embed(title = "Queued Query",
            ...
            if banreason is None:
                embed = discord.Embed(description = f"`ID: {user.id}` | {user.mention} found with the nickname: **{user.display_name}**\u0020",
    ...
    async def info_error(self, ctx, error):
        if isinstance(error, (commands.UserNotFound, commands.errors.UserNotFound)):
            embed = discord.Embed(title = "User Not Found", description = "Try using an actual user next time? :(",
